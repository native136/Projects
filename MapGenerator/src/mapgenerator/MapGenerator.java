			...
			for (int j = 0; j < height; j++) {
				if (tiles[i][j].getType() == Tile.Type.MOUNTAIN) {
					...
					if (c >= 90) {
						...
						Point closestTile = findClosestOfType(tiles,Type.SHALLOW_WATER,i, j, 301); // May have to make these as runnables to fix long map gen times
			...
			for (int x = i - 1; x <= i + 1; x++) {
				for (int y = j - 1; y <= j + 1; y++) {
					...
					if ((tiles[x][y].getType() != Tile.Type.RIVER_SOURCE)
							...
							&& ((x == i) || (y == j))
							&& (!((x == i) && (y == j)))) {
						if (getDifficulty(x, y, closestTile.x, closestTile.y, tiles) < d) {
...
//					}
	...
	private Tile[][] PlotRoads(Tile[][] tiles, int i, int j, Point closestTile) {
		...
		if ((i != 0) && (j != 0) && (i < tiles.length - 1)
			...
			for (int x = i - 1; x <= i + 1; x++) {
				for (int y = j - 1; y <= j + 1; y++) {
//					// make sure we don't go back (Could probably add a last TODO 
//					// x
//					// and y var so that rivers can join together)
//					if ((tiles[x][y].getType() != Tile.Type.RIVER_SOURCE)
//							&& (tiles[x][y].getType() != Tile.Type.RIVER)
...
//					}
	...
	// a point array
	private float getDifficulty(int x, int y, int destX, int destY, Tile[][] tiles) {
	...
	private Point findClosestOfType(Tile[][] tiles, Type type, int x, int y,
		...
		for (int px = 0; px < tiles.length; px++) {
			for (int py = 0; py < tiles[0].length; py++) {
				...
				((((px - x) * (px - x))) + ((py - y) * (py - y))) >=(radius *radius)-radius){
	...
	private Point findLowestPoint(Tile[][] tiles, int x, int y,
			int radius) {
		float d = 10000;
